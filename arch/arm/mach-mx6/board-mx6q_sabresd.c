...
#if defined(CONFIG_CAN_MCP251X)
#define CAN_IRQGPIO    IMX_GPIO_NR(2, 26)
#endif
#define SCREEN_LVDS 100
#define SCREEN_LCD  10
#define SCREEN_UNKNOWN  0

static int  screen_type0=SCREEN_UNKNOWN;
static int  screen_type1=SCREEN_UNKNOWN;
...
{
	 int dat;
	 dat=gpio_get_value(SABRESD_TS_INT);
	 return (dat==0)?1:0;
};
...
static void lvds_power_enable(void){

                gpio_request(SABRESD_LVDS_PWREN, "lvds_pwn");
                gpio_direction_output(SABRESD_LVDS_PWREN, 1);
...
static struct i2c_board_info mxc_i2c2_board_info1[] __initdata = {
/*	{
		I2C_BOARD_INFO("max17135", 0x48),
		.platform_data = &max17135_pdata,
	},*/
				   ...
				   char **cmdline, struct meminfo *mi)
{
	...
	for_each_tag(t, tags) {
  			...
  			/* touchscreen and pwm device select */
                        str = t->u.cmdline.cmdline;
                        str = strstr(str, "video=mxcfb0:dev=");
                        if (str != NULL) {
                                str += 17;
                               if(strncmp(str,"ldb",3)==0){
                                       screen_type0=SCREEN_LVDS;
                               }
                               else if(strncmp(str,"lcd",3)==0){
                                       screen_type0=SCREEN_LCD;
                               }

                        }
                          /* touchscreen and pwm device select */
                        str = t->u.cmdline.cmdline;
                        str = strstr(str, "video=mxcfb1:dev=");
                        if (str != NULL) {
                                str += 17;
                               if(strncmp(str,"ldb",3)==0){
                                       screen_type1=SCREEN_LVDS;
                               }
                               else if(strncmp(str,"lcd",3)==0){
                                       screen_type1=SCREEN_LCD;
                               }
...
static void __init mx6_sabresd_board_init(void)
{
	...
	gpio_request(SABRESD_EPDC_PWRSTAT, "uart2_en");
	gpio_direction_output(SABRESD_EPDC_PWRSTAT, 1);
	gpio_set_value(SABRESD_EPDC_PWRSTAT, 1);
...
//	imx6q_add_v4l2_capture(1, &capture_data[1]);
	...
	if(screen_type0==SCREEN_LCD)
        ...
        if((screen_type0+screen_type1)==10){
                mx6_sabresd_pwm_backlight_data.pwm_id=1;
        }else if((screen_type0+screen_type1)==110){
                mx6_sabresd_pwm_backlight_data.pwm_id=4;

        }else if((screen_type0+screen_type1)==200){
                mx6_sabresd_pwm_backlight_data.pwm_id=3;
        }else if((screen_type0+screen_type1)==100){
                mx6_sabresd_pwm_backlight_data.pwm_id=3;
        }else{
                mx6_sabresd_pwm_backlight_data.pwm_id=1;
